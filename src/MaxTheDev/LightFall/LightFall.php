...
class LightFall extends PluginBase implements Listener {
        ...
        public $prefix = TextFormat::GRAY . "[" . TextFormat::AQUA . TextFormat::BOLD . "Sky" . TextFormat::GREEN . "§aFallTeam" . TextFormat::RESET . TextFormat::GRAY . "]";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
        public $reds = [ ];
	...
	{
		  $this->getLogger()->info(TextFormat::AQUA . "SkyFall§a §bGeoZDev");
	...
	public function onDeath(PlayerDeathEvent $event){
        ...
        if($asassin instanceof Player)
        {
            ...
            foreach($jugador->getLevel()->getPlayers() as $pl)
                {
                $pl->sendMessage(TextFormat::RED . $jugador->getNameTag() . TextFormat::YELLOW . " ucciso da " . TextFormat::GREEN . $asassin->getNameTag() . TextFormat::YELLOW . ".");
                }
        $statistic = new Config($this->getDataFolder() . "/statistic.yml", Config::YAML);
        ...
        switch($cmd->getName()){
				...
				if($player->isOp())
				{
					...
					else
					{
						$player->sendMessage($this->prefix . "/lf <make-leave> : Create Arena | Leave the game");
                                                ...
                                                $player->sendMessage($this->prefix . "/lfstart : Starta un game nei prossimi 10 secondi");
					}
				}
				else
				{
			...
			return true;
                        ...
                        case "lfstart":
                            if($player->isOp())
				{
                                $player->sendMessage($this->prefix . "§bTra 10 secondi inizia la partita!");
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			if($this->mode==26)
			{
				$tile->setText(TextFormat::AQUA . "[Partecipa]",TextFormat::YELLOW  . "0 / 12","§f".$this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
			...
			else
			{
				...
				{
					if($text[0]==TextFormat::AQUA . "[Partecipa]")
					{
						$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
                                                ...
                                                if($text[1]==TextFormat::YELLOW  . "0 / 12")
					{
						...
						$player->setNameTag("§c[RED]>>" . $player->getName());
                                                ...
                                                $player->sendMessage($this->prefix . "Sei nel team §c[RED] ");
												$player->addTitle("§c[RED]");
												$doicanh = Item::get(444,0,1);
												$inv = $player->getInventory();
                                                foreach($level->getPlayers() as $playersinarena)
                                                {
                                                $playersinarena->sendMessage($player->getName() . " Team §c[RED] ");
                                                }
					}
                                        else if($text[1]==TextFormat::YELLOW  . "1 / 12")
					{
						...
						$player->setNameTag("§9[BLUE]>>" . $player->getName());
                                                ...
                                                $player->sendMessage($this->prefix . "Sei nel team §9[BLUE] ");
												$doicanh = Item::get(444,0,1);
												$inv = $player->getInventory();
												$player->addTitle("§9[BLUE]");
                                                foreach($level->getPlayers() as $playersinarena)
                                                {
                                                $playersinarena->sendMessage($player->getName() . "Team §9[BLUE] ");
                                                }
					}
                                        else if($text[1]==TextFormat::YELLOW  . "2 / 12")
					{
						...
						$player->setNameTag("§a[GREEN]>>" . $player->getName());
                                                ...
                                                $player->sendMessage($this->prefix . "Sei nel team §a[GREEN] ");
												$doicanh = Item::get(444,0,1);
												$inv = $player->getInventory();
					...
					{
						$player->sendMessage($this->prefix . "Non puoi entrare");
					}
				}
			}
		}
		...
		foreach($tiles as $t) {
			if($t instanceof Sign) {	
				...
				if($text[3]==$this->prefix)
				{
					...
					$ingame = TextFormat::AQUA . "[Partecipa]";
					...
					else
					{
						if(count($playersArena)>=2)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendTip("§e< Ancora" . TextFormat::GREEN . $timeToStart . " per iniziare §e >");
								}
                                                                if($timeToStart==89)
                                                                {
							...
							else
							{
								...
								if($aop>=1)
								{
									...
									foreach($playersArena as $pl)
									{
										...
										{
											$plpl->sendMessage($this->prefix . "Team §c[RED]§b Ha vinto su §a" . $arena);
										}
									...
									$config->set($arena . "StartTime", 180);
										...
										foreach($this->plugin->getServer()->getOnlinePlayers() as $plpl)
										{
											$player->sendMessage("+500$");
											$player->addTitle("§lWin!");
											$plpl->sendMessage($this->prefix . "§bTeam §9[BLUE]§b Ha vinto su §a" . $arena);
										}
										$pl->getInventory()->clearAll();
										$pl->removeAllEffects();
										$pl->setNameTag($pl->getName());
									...
									$config->set($arena . "StartTime", 180);
										...
										foreach($this->plugin->getServer()->getOnlinePlayers() as $plpl)
										{
											$plpl->sendMessage($this->prefix . "§bTeam §a[GREEN]§b ha vinto su §a" . $arena);
										}
										$pl->getInventory()->clearAll();
										$pl->removeAllEffects();
										$pl->setNameTag($pl->getName());
									...
									$config->set($arena . "StartTime", 180);
                                                                ...
                                                                if(($aop>=2))
                                                                    ...
                                                                    foreach($playersArena as $pl)
																		...
																		{
                                                                        $pla->sendTip("§l§7[§bSkyFallTeam§7]\n §eTime:" . $time);
                                                                        }
                                                                }
								$time--;
								if($time == 779)
								{
									...
									{
										$pl->sendMessage("§b>================================");
                                                                                $pl->sendMessage("§a La partità è iniziata, buona fortuna :)");
                                                                                $pl->sendMessage("§c Gioca seriamente per vincere!");
                                                                                $pl->sendMessage("§e Hai 15 §d secondi§e di invincibilità.");
                                                                                $pl->sendMessage("§b=================================");
																				$pl->addTitle("§bCombatti\n §cGame iniziato!");
																				$doicanh = Item::get(444,0,1);
																				$inv = $pl->getInventory();
																				$inv->addItem($doicanh);
								...
								if($time == 750)
								{
									...
									{
										$pl->sendMessage("§e>§e-------------------");
                                                                                $pl->sendMessage("§e>§bBeacon§6Games §cNetwork");
                                                                                $pl->sendMessage("§e>§e-------------------");
									}
								}
                                                                if($time == 550)
								{
									...
									{
										$pl->sendMessage("§e>§e--------------------------");
                                                                                $pl->sendMessage("§e>§b Sta attento! :)");
                                                                                $pl->sendMessage("§e>§e--------------------------");
									}
								}
                                                                if($time == 480)
								{
									...
									{
										$pl->sendMessage("§e>§e--------------------------");
                                                                                $pl->sendMessage("§e>§bChest riempite");
                                                                                $pl->sendMessage("§e>§e--------------------------");
																				$pl->addTitle("§eLe chest sono state\n §aRiempite");
									}
									$this->refillChests($levelArena);
								}
								if($time>=180)
								...
								else
								{
									...
									if($time <= 0)
									{
										...
										{
											$pl->teleport($spawn,0,0);
											$pl->sendMessage($this->prefix . "Nessuna squadra ha vinto su §a" . $arena);
											$pl->getInventory()->clearAll();
                                                                                        $pl->setHealth(20);
                                                                                        $pl->setNameTag($pl->getName());
                                                                                        $this->getResetmap()->reload($levelArena);
						...
						else
						{
							if($timeToStart<=0)
							{
								foreach($playersArena as $pl)
								{
									foreach($this->plugin->getServer()->getOnlinePlayers() as $plpl)
									{
										$plpl->sendMessage($this->prefix . $pl->getNameTag() . "§l§b ha vinto su §a" . $arena);
									}
									$spawn = $this->plugin->getServer()->getDefaultLevel()->getSafeSpawn();
									$this->plugin->getServer()->getDefaultLevel()->loadChunk($spawn->getX(), $spawn->getZ());
									$pl->getInventory()->clearAll();
							...
							else
							{
								foreach($playersArena as $pl)
								{
									$pl->sendTip(TextFormat::DARK_AQUA . "In attesa di giocatori..");
								}
								$config->set($arena . "PlayTime", 780);
								$config->set($arena . "StartTime", 90);
							}
